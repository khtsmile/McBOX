...
subroutine GET_SAB_MIC(mat,imat,erg,xs)
    ...
    ipfac = max(0D0,min(1D0,(erg-abi%erg(ierg)) &
        /(abi%erg(ierg+1)-abi%erg(ierg))))
    xs(1) = abi%xs(ierg) + ipfac*(abi%xs(ierg+1)-abi%xs(ierg))
    ...
    ipfac = max(0D0,min(1D0,(erg-abe%erg(ierg)) &
        /(abe%erg(ierg+1)-abe%erg(ierg))))
    xs(2) = abe%xs(ierg) + ipfac*(abe%xs(ierg+1)-abe%xs(ierg))
    if ( sab(isab)%nxs(5) == 4 ) xs(2) = xs(2) / abe%erg(ierg)
    end if

    ! total
    xs(1) = xs(1) + xs(2) + xs(3)   ! total = inelastic + elastic + absorption
